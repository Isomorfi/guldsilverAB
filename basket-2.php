...
function calculateGoldSilverCount($conn, &$goldCount, &$silverCount, $orderID) {
	...
	if($goldCount > 0) {
		$sql = "INSERT INTO db19880310.OrderItems (OrderID, ProductID, Quantity, TotalCost)
			VALUES ('$orderID', '1', $goldCount, $priceGold)";
		$res = mysqli_query($conn, $sql);	
	}
	...
	if($silverCount > 0) {	
		$sql = "INSERT INTO db19880310.OrderItems (OrderID, ProductID, Quantity, TotalCost)
			VALUES ('$orderID', '2', $silverCount, $priceSilver)";
		$res = mysqli_query($conn, $sql);
	}
